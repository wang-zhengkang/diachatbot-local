...
def main():
        ...
        for round in dialogue['messages']:
            ...
            for act in round['dialog_act']:
                ...
                if '酒店设施' in cur_act[2]:
                    facility = cur_act[2].split('-')[1] #  ["Inform","酒店","酒店设施-健身房","是"]
                    if cur_act[0] == 'Inform':
                        cur_act[2] = cur_act[2].split('-')[0] + '+' + cur_act[3] #  酒店设施+是  
                    elif cur_act[0] == 'Request':
                        cur_act[2] = cur_act[2].split('-')[0]  # 酒店设施
                if cur_act[0] == 'Select':   # ["Select","景点","源领域","酒店"]
                    cur_act[2] = '源领域+' + cur_act[3] # 源领域+酒店
                    
                intent = '+'.join(cur_act[:-1]) #取0，1，2三个值作为intent，+作为间隔，比如：Inform+酒店+酒店类型
                ...
                if (act[0] in ['Inform', 'Recommend'] or '酒店设施' in intent) and not intent.endswith('无'):
                    if act[3] in content or (facility and facility in content): #如果value在语句里，替换成placeholder
            ...
            if usable:
                if round['role'] == 'usr':
                    ...
                    sys_multi_intent_dict[multi_intent].append(content)  #和模板生成差不多
    ...
    for require_role in ['sys', 'usr']:
        ...
        # ### vocab.txt
        #把模板中的语句分词，得到vocab，分词前，先把intent替换出来
        vocab_dict = defaultdict(int)
        pattern = re.compile(r'(\[[^\[^\]]+\])')#^:当该符号在方括号表达式中使用时，表示不接受该方括号表达式中的字符集合
        ...
        vocab_dict = {word: frequency for word, frequency in vocab_dict.items() if vocab_dict[word] > 3}#保留词频超过3的词
        len(vocab_dict)
        ...
        '''
        sen是模板的语句：你好，这个是[Inform+酒店+酒店类型]的酒店。
                   处理逻辑就是把intent替换掉，然后分词，再把分词后的词按照原来的顺序和intent拼接到一起，用空格间隔
                    处理结果就是： 你好 ， 这个 是 [Inform+酒店+酒店类型] 的 酒店 。
        '''
            ...
            for idx, dialogue in data[split].items():
                ...
                for round in dialogue['messages']:
                    ...
                    for act in round['dialog_act']:
                        ...
                        feat_dict[idx][round_id] = feat_dict[idx].get(round_id, dict()) #初始化
                        feat_dict[idx][round_id][feat_key] = feat_dict[idx][round_id].get(feat_key, []) #初始化
