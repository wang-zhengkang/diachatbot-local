...
import sys
sys.path.append(r"D:\convlab project\diabetes220311")
from convlab2.nlg.sclstm.diachat import SCLSTM
...
def act2intent(dialog_act: list):
    # print('dialog_act为：',dialog_act)
    cur_act = copy.deepcopy(dialog_act)
    for ele in dialog_act['slot_values']:  # slot_values对应的列表中可能有多个字典
        intent = '+'.join([dialog_act['act_label'], ele['domain'], ele['slot']])
...
def value_replace(sentences, dialog_act):
    ...
    for act in dialog_act:
        ...
        # sentences = sentences.replace('[' + intent + '1]', act[3])


        for ele in act['slot_values']:
            sentences = sentences.replace('[' + intent + ']', ele['value'])
            sentences = sentences.replace('[' + intent + '1]', ele['value'])  # if multiple same intents and this is 1st
...
if __name__ == '__main__':
    if len(sys.argv) != 2:
        ...
        print("\t data_key=User/Doctor")
        sys.exit()
    data_key = sys.argv[1]
    print(data_key,data_key == 'Doctor')
    assert data_key=='User' or data_key=='Doctor'
    model = SCLSTM(is_user=(data_key=='User'),use_cuda=True)
    ...
    if os.path.isfile('dialog_acts_%s.pkl' % data_key) and os.path.isfile(
        ...
        for sess in test_data:
            ...
            for turn in sess['utterances']:
                if turn['agentRole'] == 'User' and data_key == 'Doctor':
                    continue
                elif turn['agentRole'] == 'User' and data_key == 'Doctor':
                    continue
                sen_num += 1
        ...
        for sess in test_data:
            ...
            for turn in sess['utterances']:
                ...
                dialog_acts.append(turn['annotation'])
                golden_utts.append(turn['utterance'])  # slots **values**
                gen_utt = model.generate(turn['annotation']) # 研究generate函数
    ...
    """
        for i in range(5):
        print(gen_utts[i])
为您推荐鲜鱼口老字号美食街，人均消费75元。
营业时间是周一至周日 10:00-22:00。
周边景点有故宫,天安门广场,前门大街,恭王府。
故宫的评分是4.7分。
电话是010-85007938，地址是北京市东城区景山前街4号。
    """
