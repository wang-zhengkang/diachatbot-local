...
def read(config, args, mode):
    ...
    dataset = DatasetDiachat(config, percentage=percentage, use_cuda=USE_CUDA)#加载数据
...
def train(config, args):
    dataset, model = read(config, args, args.mode)#加载训练数据
    n_layer = args.n_layer
    model_path = args.model_path
...
def interact(config, args):
    dataset = SimpleDatasetDiachat(config)
...
def parse():
    ...
    parser.add_argument('--model_path', type=str, default='sclstm0.pt', help='saved model path')
    ...
    if args.user:
        config.read(CURRENT_DIR + 'config/config_usr.cfg')
    else:
        config.read(CURRENT_DIR + 'config/config.cfg')
    config.set('DATA', 'dir', os.path.dirname(os.path.abspath(__file__)))

    return args, config

#--mode=train --model_path=sclstm0.pt --n_layer=2 --lr=0.005
#--mode=train --model_path=sclstm_usr0.pt --n_layer=2 --lr=0.005 --user True
if __name__ == '__main__':# 关注输入的X和Y，损失函数？lstm
    ...
    args.mode = 'train'
    args.model_path = config['DATA']['dir'] + '/sclstm-0509.pt'
    args.n_layer = 4
    args.lr = 0.005
