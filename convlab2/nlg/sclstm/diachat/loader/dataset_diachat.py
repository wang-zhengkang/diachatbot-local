...
class DatasetDiachat(object):
    ...
    def __init__(self, config, percentage=1.0, use_cuda=False):
        # setup
        feat_file = config['DATA']['feat_file'] #config中
...
#             print("len(self.word2index)=",len(self.word2index))
            res = []
        ...
        #add by yangjinfeng
#         print("len(sentences_padded)=",len(sentences_padded))
        i=0
        for s in sentences_padded:
            i = i +1
            if i >=10:
                break
#             print("len(s)=",len(s))
    ...
    def _setCardinality(self, template_file):
        ...
        with open(template_file,encoding='utf-8') as f:
            ...
            for i in range(0, len(self.dfs) - 1):
                self.dfs[i + 1] = self.dfs[i] + self.dfs[i + 1]#排序？1.2.3.4...
    ...
    def _setupData(self, text_file, feat_file, dataSplit_file):
        ...
        for data_type in ['train', 'valid', 'test']:
            for dial_idx, turn_idx, _ in dataSet_split[data_type]:
                ...
                if turn_idx not in dial2meta[str(dial_idx)]:
                    #by yangjinfeng
                    print("dial_id:{}, turn_id:{}".format(dial_idx,turn_idx))
                    continue

                meta = dial2meta[str(dial_idx)][turn_idx]
                text = dial2text[str(dial_idx)][turn_idx]
#                 text = dial2text[dial_idx][turn_idx]
                self.data[data_type].append((str(dial_idx), turn_idx, text, meta))
    ...
    def getFeatIdx(self, meta):
        # 输入： {'餐馆-Inform': [['人均消费', '1', '100-150元']], '餐馆-Request': [['电话', '?', '?']]}
        feat_container = []
        do_idx, da_idx, sv_idx = [], [], []
        for da, slots in meta.items():
            do = da.split('-')[0]
#             print('dataset_woz.py文件中getFeatIdx中的', 'do为', do)
            _do_idx = self.cardinality.index('d:' + do) - self.dfs[0]
#             print('dataset_woz.py文件中getFeatIdx中的','_do_idx为',_do_idx)
            """
            dataset_woz.py文件中getFeatIdx中的 do为 餐馆
dataset_woz.py文件中getFeatIdx中的 _do_idx为 0
dataset_woz.py文件中getFeatIdx中的 do为 餐馆
dataset_woz.py文件中getFeatIdx中的 _do_idx为 0
            """
...
class SimpleDatasetDiachat(DatasetDiachat):
    def __init__(self, config):
        vocab_file = config['DATA']['vocab_file']
        template_file = config['DATA']['template_file']#['vocab_file']、['template_file']需要我们自己做吗
        self.batch_size = 1
