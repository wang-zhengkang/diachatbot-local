...
class TemplateNLG(NLG):
    ...
    def _auto_generate(self, dialog_act, template):
        ...
        #add by yangjinfeng
        print(multi_intent," 是否存在： ", (multi_intent in template))
        try:
    ...
    def _prepare_intent_string_list(self, dialog_act):
        ...
        if self.mode == 'manual':
            ...
            #add by yangjinfeng
            print("intent list from dialog acts: ",intent_list)
            return copy.copy(intent_list)
...
#     print(nlg.generate([['Inform', '地铁', '目的地', '云峰山'], ['Request', '地铁', '出发地', '']]))
    ...
    print(nlg.generate([['Inform', '地铁', '目的地', '云峰山'], ['Request', '地铁', '出发地附近地铁站', '']]))
