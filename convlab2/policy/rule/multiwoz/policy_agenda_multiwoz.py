...
# REF_USR_DA {'domain': {'slot': 'Slot'}}
# REF_SYS_DA {'domain': {'Slot': 'slot'}}
# SYS和USR的slot不完全一致
REF_USR_DA_M = copy.deepcopy(REF_USR_DA)
REF_SYS_DA_M = {}
# 该for循环把SYS里所有的Key-Value字母都改成小写
...
class UserPolicyAgendaMultiWoz(Policy):
    ...
    # load stand value
    # 存储所有value，形式为dict: domain-slot-value
...
if __name__ == '__main__':
    ...
    sys_act = ['Inform', 'Hotel', 'Price', '100-200']
    print(sys_act)
    # #
    user_act = user_policy.predict(sys_act)
    print(user_act)
