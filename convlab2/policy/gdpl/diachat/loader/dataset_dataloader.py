...
from convlab2.policy.gdpl.diachat.processed_data.state_structure import *
belief_state_mould = default_state()['belief_state']
for sort in ['current', 'AskForSure', 'AskFor', 'AskHow', 'AskWhy']:
    belief_state_mould[sort] = {}
    for domain in ['问题', '饮食', '行为', '运动', '治疗', '基本信息']:
        if domain == '基本信息':
            belief_state_mould[sort][domain] = {
                '身高': [],
                '体重': [],
                '年龄': [],
                '性别': [],
...
class DiachatDataloader(DatasetDataloader):
    ...
    def load_data(self,
        ...
        # 获取[a,d,s,v]同时更新belief_state
        # def da2tuples(dialog_act, belif_state):
        def da2tuples(dialog_act):
            # def org(da_temp: list, dsv: dict, act_label, belif_state):
            def org(da_temp: list, dsv: dict, act_label):
                domain = dsv['domain'] if dsv['domain'] else 'none'
                slot = dsv['slot'] if dsv['slot'] else 'none'
                value = dsv['value'] if dsv['value'] else 'none'
                key = act_label
                da_temp.append(key)
                da_temp.append(domain)
                da_temp.append(slot)
                da_temp.append(value)
                # if key in ['Inform', 'AskForSure', 'AskFor', 'AskHow', 'AskWhy']:
                #     sort = 'current' if key == 'Inform' else key
                # belif_state[sort][domain][slot].append(value)
            ...
            for da in dialog_act:
                ...
                for dsv in da['slot_values']:
                    da_temp = []
                    # org(da_temp, dsv, act_label, belif_state)
                    org(da_temp, dsv, act_label)
                    if da_temp not in tuples:
                        tuples.append(da_temp)
            return tuples

        assert role in ['sys', 'usr', 'all']
        info_list = list(filter(eval, ['utterance', 'dialog_act', 'context', 'context_dialog_act', 'belief_state',
                                       'last_opponent_utterance', 'last_self_utterance', 'session_id',
                                       'span_info', 'terminated', 'goal']))
        self.data = {'train': {}, 'val': {}, 'test': {}, 'role': role, 'human_val': {}}
        dst = RuleDST()
        if data_key == 'all':
            data_key_list = ['train', 'val', 'test']
        ...
        for data_key in data_key_list:
            ...
            for session in data:
                cur_context = []
                cur_context_dialog_act = []
                sess_id = session['conversationId']
                dst.init_session()
                # belief_state_temp = deepcopy(belief_state_mould)
                for i, turn in enumerate(session['utterances']):
                    text = turn['utterance']
                    # da = da2tuples(turn['annotation'], belief_state_temp)
                    da = da2tuples(turn['annotation'])
                    ...
                    if span_info:
                        self.data[data_key]['span_info'].append(turn['span_info'])
                    if terminated:
                        self.data[data_key]['terminated'].append(i + 2 >= len(session['utterances']))
                    if goal:
                        self.data[data_key]['goal'].append(session['goal'])
                    cur_context.append(text)
                    cur_context_dialog_act.append(da)
        if ontology:
            ontology_path = os.path.join(data_dir, 'ontology.json')
