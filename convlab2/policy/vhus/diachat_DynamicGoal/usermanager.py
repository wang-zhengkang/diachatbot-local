...
class UserDataManager(object):
    ...
    def vocab_loader(self):
        if self.__voc_goal is None or self.__voc_usr is None or self.__voc_usr_rev is None or self.__voc_sys is None:
            vocab_path = 'data/diachat/goal/vocab_dynamic.pkl'
    ...
    def org_data_loader(self):
        ...
        if self.__org_goals is None or self.__org_usr_dass is None or self.__org_sys_dass is None:
            ...
            self.__org_goalss = [[UserDataManager.usrgoal2seq(goal) for goal in goals] for goals
                                   in goalss]
            self.__org_usr_dass = [[UserDataManager.usrda2seq(usr_da, goal) for (usr_da, goal) in zip(usr_das, goals)] for (usr_das, goals)
                                   in zip(usr_dass, goalss)]
            self.__org_sys_dass = [[UserDataManager.sysda2seq(sys_da, goal) for (sys_da, goal) in zip(sys_das, goals)] for (sys_das, goals)
                                   in zip(sys_dass, goalss)]
