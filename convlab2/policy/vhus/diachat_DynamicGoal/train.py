...
from convlab2.policy.vhus.diachat_DynamicGoal.usermodule import VHUS
from convlab2.policy.vhus.diachat_DynamicGoal.util import padding_data, kl_gaussian
...
class VHUS_Trainer():
    ...
    def save(self, directory, fold, is_train_all=False):
        if is_train_all:
            ...
            torch.save(self.user.module.state_dict(), directory + '/' + f'{fold}' + '_simulator.mdl')
            logging.info('<<US>> train all data module saved network to mdl')
        else:
            if not os.path.exists(directory):
                os.makedirs(directory)
            torch.save(self.user.module.state_dict(), directory + '/' + str(fold) + '_simulator.mdl')
            logging.info('<<US>> saved {} fold network to mdl'.format(fold))
