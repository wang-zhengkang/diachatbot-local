...
from convlab2.policy.vhus.diachat_StaticGoal.util import padding_data, kl_gaussian
...
class VHUS_Trainer():
    def __init__(self, config, manager, goal_gen, fold, train_idx, test_idx):
        ...
        epoch = config['epoch']
        lr = config['lr']
        hu_dim = config['hu_dim']
        eu_dim = config['eu_dim']
        alpha = config['alpha']
    ...
    def imitating(self, epoch, is_train_all=False):
        ...
        if is_train_all:
            self.save(self.save_dir, epoch, is_train_all)
    ...
    def imit_test(self, epoch, best):
        ...
        loss = a_loss + t_loss
        if loss < best:
            logging.info('<<US>> best model saved')
            best = loss
            self.save(self.save_dir, f'{self.fold}_fold_best')
    ...
    def save(self, directory, epoch, is_train_all=False):
        if is_train_all:
            if not os.path.exists(directory):
                os.makedirs(directory)
            torch.save(self.user.module.state_dict(), directory + '/' + 'all_data' + '_simulator.mdl')
            logging.info('<<US>> train all data module saved network to mdl')
        else:
            if not os.path.exists(directory):
                os.makedirs(directory)
            torch.save(self.user.module.state_dict(), directory + '/' + str(epoch) + '_simulator.mdl')
            logging.info('<<US>> epoch {}: saved network to mdl'.format(epoch))
