    ...
    def __init__(self):
        ...
        for domain in domains:
            with open(os.path.join(os.path.dirname(
                    ...
                    'data/diachat/db/{}_db.json'.format(domain))) as f:
    ...
    def query(self, domain, constraints, ignore_open=False, soft_contraints=(), fuzzy_match_ratio=60):
        ...
        if domain == 'hospital':
            ...
            if not department:
                ...
                return [deepcopy(x) for x in self.dbs['hospital'] if
                        x['department'].lower() == department.strip().lower()]
        constraints = list(
            map(lambda ele: ele if not (ele[0] == 'area' and ele[1] == 'center') else ('area', 'centre'), constraints))
                    ...
                    try:
                        ...
                        # elif ignore_open and key in ['destination', 'departure', 'name']:
                            ...
                            if not fuzzy_match:
                                ...
                                if fuzz.partial_ratio(val.strip().lower(),
                                                      record[key].strip().lower()) < fuzzy_match_ratio:
                                    break
                    except:
                        continue
            else:
...
if __name__ == '__main__':
    db = Database()
    print(db.query("train", [['departure', 'cambridge'], ['destination', 'peterborough'], ['day', 'tuesday'],
                             ['arriveBy', '11:15']]))
