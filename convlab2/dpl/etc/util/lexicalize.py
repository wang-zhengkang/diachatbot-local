...
# ['Inform+餐馆+名称+1'] 餐馆 db_res的数据库
def lexicalize_da(da, cur_domain, entities):
    not_dish = {'当地口味', '老字号', '其他', '美食林风味',
                '特色小吃', '美食林臻选', '深夜营业', '名人光顾', '四合院'}
    lexicalized_da = []
    for a in da:
                ...
                if intent == 'Recommend':
                    ...
                    if len(entities) > value:
                        ...
                        if '周边' in slot:
                            ...
                            assert isinstance(entity[slot], list)
                            dishes = [x for x in entity[slot]
                                      if x not in not_dish]
                            if len(dishes) > value:
                                value = dishes[value]
                            lexicalized_da.append(
                                [intent, domain, slot, value])
                            ...
                            value = entity[slot]
                            lexicalized_da.append(
                                [intent, domain, slot, '{}元'.format(value)])
                        elif slot == '评分':
                            assert value == 0
                            value = entity[slot]
                            lexicalized_da.append(
                                [intent, domain, slot, '{}分'.format(value)])
                        else:
                            assert value == 0
                            value = entity[slot]
                            lexicalized_da.append(
