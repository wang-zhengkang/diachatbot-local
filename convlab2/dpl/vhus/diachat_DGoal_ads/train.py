...
from convlab2.policy.vhus.diachat_DGoal_ads.util import padding_data, kl_gaussian
...
class VHUS_Trainer():
    ...
    def save(self, directory, fold, is_train_all=False):
        if is_train_all:
            ...
            logging.info('train all data module saved network to mdl')
        else:
            if not os.path.exists(directory):
                os.makedirs(directory)
            torch.save(self.user.module.state_dict(), directory + '/' + str(fold) + '_fold_simulator.mdl')
            logging.info('saved {} fold network to mdl'.format(fold))
