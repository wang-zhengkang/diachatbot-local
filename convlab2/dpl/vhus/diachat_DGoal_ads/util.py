...
def padding_data(data):
    ...
    # goal
    for sess in batch_goals:
        goals_length += [len(sen) for sen in sess]
    max_goal_length = max(goals_length)  # R
    for sess in batch_goals:
        try:
            goals += padding(sess, max_goal_length)
        except:
            goals = padding(sess, max_goal_length)
            
    # terminated
    for l in sentence_num:
        terminated += [0] * (l - 1) + [1]
...
def kl_gaussian(argu):
    ...
    loss -= (recog_logvar.exp() + torch.pow(recog_mu -
             prior_mu, 2)) / prior_logvar.exp()
