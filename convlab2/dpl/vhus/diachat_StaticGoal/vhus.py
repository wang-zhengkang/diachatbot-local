...
class UserPolicyVHUSAbstract(Policy):
    ...
    def init_session(self):
        ...
        self.list_form_goal = self.goal_gen.get_user_list_form_goal()
        self.goal_input = torch.LongTensor(self.manager.get_goal_id(self.manager.usrgoal2seq(
            self.list_form_goal))).cuda(device=device)
        self.goal_len_input = torch.LongTensor(
            [len(self.goal_input)]).squeeze().cuda(device=device)
        self.sys_da_id_stack = []  # to save sys da history
        self.terminated = False
        # 为每个sort-domain-slot目标创建value迭代器
        # for sort, sort_dic in self.goal.items():
        #     if sort_dic:
        #         for domain, slot_dic in sort_dic.items():
        #             if slot_dic:
        #                 for slot, value_list in slot_dic.items():
        #                     exec(f'self.{sort}_{domain}_{slot}=iter({value_list})')
    ...
    def load(self, archive_file, model_file, filename):
        ...
        user_mdl = os.path.join(os.path.dirname(
            os.path.abspath(__file__)), filename + '_simulator.mdl')
        if os.path.exists(user_mdl):
            DEVICE = torch.device(
                "cuda" if torch.cuda.is_available() else "cpu")
            self.user.load_state_dict(
                torch.load(user_mdl, map_location=DEVICE))
            print('<<US>> loaded checkpoint from file: {}'.format(user_mdl))

    def load_from_local_path(self, path):
        if os.path.exists(path):
            self.user.load_state_dict(torch.load(path))
            print('<<US>> loaded checkpoint from file: {}'.format(path))
