...
class UserDataManager(object):
    ...
    def usrgoal2seq(goal: dict):
        ...
        for sort in SORTS:
            if goal[sort]:
                ret.append(sort)
                ret.append('(')
                for dsv_list in goal[sort]:
                    
                    ret.append(dsv_list[0])
                ret.append(')')
        return ret
    ...
    def vocab_loader(self):
        if self.__voc_goal is None or self.__voc_usr is None or self.__voc_usr_rev is None or self.__voc_sys is None:
            vocab_path = 'data/diachat/goal/vocab_static.pkl'
    ...
    def sysda2seq(sys_da: dict, goal: dict):
        ...
        # NotInGoal:
        # InConstraint: current
        # InRequest: AskForSure, AskFor, AskHow, AskWhy
        for act, dsv_list in sys_da.items():
            ...
            for dsv in dsv_list:
                ...
                # if dsv == ['none-none', 'none']:
                #     break
                # else:
                #     for sort in SORTS:
                #         if dsv in goal[sort]:
                #             if sort == 'current':
                #                 ret.append(dsv[0] + '=' + 'InConstraint')
                #             elif sort in ['AskForSure', 'AskFor', 'AskHow', 'AskWhy']:
                #                 ret.append(dsv[0] + '=' + 'InRequest')
                #             else:
                #                 ret.append(dsv[0] + '=' + 'NotInGoal')
                # ret.append(')')
        return ret
    ...
    @staticmethod
    def kfold_date_process(goals_seg, usr_dass_seg, sys_dass_seg, train_idx, val_idx):
        def dr(dss):
            return np.array([d for ds in dss for d in ds])
        idx_train = train_idx
        idx_val = val_idx
        return dr(np.array(goals_seg)[idx_train]), dr(np.array(usr_dass_seg)[idx_train]), dr(
            np.array(sys_dass_seg)[idx_train]), dr(np.array(goals_seg)[idx_val]), dr(np.array(usr_dass_seg)[idx_val]), dr(np.array(sys_dass_seg)[idx_val])