...
from convlab2.policy.vhus.diachat_StaticGoal.train import VHUS_Trainer

if __name__ == '__main__':
    with open('convlab2/policy/vhus/diachat_StaticGoal/config.json', 'r') as f:
        cfg = json.load(f)
    is_train_all = True
    ...
    if is_train_all:
        ...
        for fold, (train_idx, val_idx) in enumerate(skf.split(data_idx)):
            fold += 1
            logging.info(f"--------------第{fold}折交叉训练--------------")
            env = VHUS_Trainer(cfg, manager, goal_gen, fold, train_idx, val_idx)
            # best = float('inf')
            for e in range(cfg['epoch']):
                env.imitating(e)
        ...
        is_train_all = True
        env = VHUS_Trainer(cfg, manager, goal_gen, fold='all', data_idx=data_idx, val_idx = range(0))
        for e in range(cfg['epoch']):
            env.imitating(e, is_train_all)
