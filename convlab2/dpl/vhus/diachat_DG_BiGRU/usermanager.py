...
from convlab2.dpl.etc.util.domain_act_slot import SORTS
...
class UserDataManager(object):
    ...
    def vocab_loader(self):
        if self.__voc_goal is None or self.__voc_usr is None or self.__voc_usr_rev is None or self.__voc_sys is None:
            vocab_path = 'convlab2/dpl/etc/data/voc_VHUS.pkl'
            if os.path.exists(vocab_path):
                with open(vocab_path, 'rb') as f:
                    self.__voc_goal, self.__voc_usr, self.__voc_sys = pickle.load(f)
                print('loaded voc file from: convlab2/dpl/etc/data/voc_VHUS.pkl')
            else:
                ...
                print('built voc file to: convlab2/dpl/etc/data/voc_VHUS.pkl')
            # 缩进分割线
