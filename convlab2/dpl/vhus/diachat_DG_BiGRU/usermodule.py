...
class VHUS(nn.Module):
    def __init__(self, cfg, voc_goal_size, voc_usr_size, voc_sys_size):
        ...
        self.is_bi_encoder = True
        self.is_bi_decoder = False
        self.goal_encoder = Encoder(voc_goal_size, cfg['eu_dim'], cfg['hu_dim'])
        self.sys_encoder = Encoder(voc_sys_size, cfg['eu_dim'], cfg['hu_dim'])
        self.context_encoder = nn.GRU(cfg['hu_dim'], cfg['hu_dim'], batch_first=True, bidirectional=self.is_bi_encoder)

        

        if self.is_bi_encoder:
            self.mu_net = nn.Linear(cfg['hu_dim'] * 2, cfg['hu_dim'] * 2)
            self.logvar_net = nn.Linear(cfg['hu_dim'] * 2, cfg['hu_dim'] * 2)
            self.mu_net_last = nn.Linear(cfg['hu_dim'] * 2, cfg['hu_dim'] * 2)
            self.logvar_net_last = nn.Linear(cfg['hu_dim'] * 2, cfg['hu_dim'] * 2)
            self.terminated_net = nn.Sequential(nn.Linear(cfg['hu_dim'] * 2, cfg['hu_dim']),
                                                nn.ReLU(),
                                                nn.Linear(cfg['hu_dim'], 1))
            self.concat_net = nn.Linear(cfg['hu_dim'] * 4, cfg['hu_dim'])
        else:
        ...
        self.usr_decoder = Decoder(voc_usr_size, cfg['max_ulen'], cfg['eu_dim'], cfg['hu_dim'], bidirectional=self.is_bi_decoder)
    ...
    def forward(self, goals, goals_length, posts, posts_length, origin_responses=None):
        ...
        goal_h = goal_h.unsqueeze(0)
        if self.is_bi_encoder:
            goal_h = goal_h.repeat(2, 1, 1)
        context_output, _ = self.context_encoder(post_h, goal_h)  # [B, S, H]
...
class Encoder(nn.Module):
    def __init__(self, vocab_size, embed_size, hidden_size, input_dropout_p=0, dropout_p=0, n_layers=1,
                 rnn_cell='GRU', variable_lengths=False, embedding=None, update_embedding=True, bidirectional=False):
        ...
        self.rnn = self.rnn_cell(embed_size, hidden_size, n_layers, batch_first=True,
                                 dropout=dropout_p, bidirectional=self.bidirectional)
...
class Decoder(nn.Module):
    ...
    def __init__(self, vocab_size, max_len, embed_size, hidden_size, sos_id=2, eos_id=3, n_layers=1, rnn_cell='GRU',
                 input_dropout_p=0, dropout_p=0, use_attention=False, bidirectional=False):
    ...
    def forward_step(self, input_var, hidden, encoder_outputs, function):
        ...
        if self.bidirectional:
            predicted_softmax = function(self.out(output.contiguous().view(-1, self.hidden_size * 2)), dim=1).view(batch_size,
                                                                                                           output_size,
                                                                                                           -1)
        else:
