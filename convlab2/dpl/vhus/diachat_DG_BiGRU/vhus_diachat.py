...
import json
import torch
from convlab2.dpl.etc.util.goal_generator import GoalGenerator
from convlab2.dpl.vhus.diachat_DG_BiGRU.usermanager import UserDataManager
from convlab2.dpl.vhus.diachat_DG_BiGRU.usermodule import VHUS
from convlab2.dpl.vhus.diachat_DG_BiGRU.vhus import UserPolicyVHUSAbstract
from convlab2.dpl.vhus.diachat_DG_BiGRU.util import padding
...
class UserPolicyVHUS(UserPolicyVHUSAbstract):
    def __init__(self, is_load_model=False):
        ...
        if is_load_model:
            model_file = 'convlab2/dpl/vhus/diachat_DG_BiGRU/save/all_data_simulator.mdl'
            self.user.load_state_dict(torch.load(model_file))
            print(f"loaded VHUS model file from: {model_file}")
    ...
    def predict(self, sys_action=None):
        ...
        if sys_action == None:
            ...
            sys_seq_turn = self.manager.sysda2seq(
                self.manager.da_list_form_to_dict_form(sys_action))
