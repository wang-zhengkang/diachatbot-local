...
class GoalGenerator:
    ...
    def _build_goal_model(self):
        ...
        # diachat.goal里的"domains"
        domain_orderings = []
        # 计算各复合领域占比
        for d in dialogs:
            d_domains = _get_dialog_domains(dialogs[d])  # 当前dialog涉及的domain
            first_index = []  # message列表中第一次涉及当前domain的索引
            for domain in d_domains:
                # 获取列表形式的message，message为goal的自然语言描述
        ...
        self.domain_ordering_dist = deepcopy(domain_ordering_cnt)  # 各复合领域占比
        ...
        ind_slot_value_cnt = dict([(domain, {}) for domain in domains])  # 独立槽值计数器
        domain_cnt = Counter()  # 领域计数器
        book_cnt = Counter()  # book计数器
        self.slots_combination_dist = {domain: {} for domain in domains}  # 复合slots分布
        self.slots_num_dist = {domain: {} for domain in domains}  # slots数量分布
        ...
        # ind_slot_dist: slot所在整个domain的占比; ind_slot_value_dist: value所在slot的占比
        # domain_ordering_dist: 各复合领域占比
...
if __name__ == '__main__':
    goal_generator = GoalGenerator(corpus_path=os.path.join(get_root_path(), 'data/multiwoz/train.json'),
