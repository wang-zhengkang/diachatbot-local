...
pages 808–819, Florence, Italy. Association for Computational Linguistics.
...
class CrossWOZTRADE(TRADE, nn.Module):
    ...
    #user_act是上一轮用户的语句，实际update方法并没有使用这个输入，而是使用的是self.state['history']，即所有的聊天记录
    def update(self, user_act):
        ...
        sentence = self.state['history'][-1][1].strip() #是上一轮语句，与输入的user_act相同
