...
class DiachatTRADE(TRADE, nn.Module):
    def __init__(self, argpath=None, mode='cn'):
        super(TRADE, self).__init__()

        if argpath is None:
            argpath = 'model/TRADE-multiwozdst/pr'
        args['path'] = argpath
        self.init_session()
        ...
        if path != 'model/TRADE-multiwozdst/pr':
            ...
            self.encoder.load_state_dict(trained_encoder.state_dict())  # demo时候改成了这个
        ...
        for j, data_dev in pbar:
            ...
            for bi in range(batch_size):
                ...
                if set(data_dev["turn_belief"][bi]) != set(predict_belief_bsz_ptr) and args["genSample"]:
                    ...
                    print("正确预测",set(predict_belief_bsz_ptr) )
                    print("答案为", set(data_dev["turn_belief"][bi]),"\n")
    ...
    def update(self, user_act):
        ...
        for j, data_dev in enumerate(dev):
            ...
            for bi in range(1):
                ...
                all_prediction[data_dev["ID"][bi]][data_dev["turn_id"][bi]]["pred_bs_ptr"] = predict_belief_bsz_ptr
                self.state['belief_state'] = self.reformat_belief_state(predict_belief_bsz_ptr,
...
if __name__ == '__main__':
    print(sentseg("你好，我是医生。\t你好，我是患者。 我有糖尿病"))
