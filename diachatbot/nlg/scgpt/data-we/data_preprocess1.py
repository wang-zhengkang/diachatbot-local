...
with open(os.path.join(data_dir, 'test.json'),"r", encoding='utf8') as f:
    data = json.load(f)
    ...
    for diags in utterances:
        ...
        if diags['annotation'] != '':
            if diags['agentRole'] == 'User':
                ...
                turn['da'] = diags['annotation'][-1]['act_label']
                turn['slots'] = ''
...
write_file(os.path.join(cur_dir, 'data/sys_test1'), sys_results)
write_file(os.path.join(cur_dir, 'data/user_test1'), user_results)
